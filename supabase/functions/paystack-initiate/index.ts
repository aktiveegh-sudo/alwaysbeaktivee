...
Deno.serve(async (req) => {
  try {
    ...
    if (!orderId || !returnUrl) {
      return json({ success: false, error: "order_id and return_url are required.", rawBody: rawText });
    }
    ...
    if (!paystackResp.ok || !paystackBody?.status) {
      return json({ success: false, error: paystackBody?.message || "Paystack initialization failed.", details: paystackBody });
    }
